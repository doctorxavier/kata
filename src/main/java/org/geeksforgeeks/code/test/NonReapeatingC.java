...
public final class NonReapeatingC {

	private static final int MAX_CHAR = 256;

	private NonReapeatingC() {
	...
	public static void findFirstNonRepeating() {
		...
		final List<Character> inDLL = new ArrayList<Character>();

		// repeated[x] is true if x is repeated two or more times.
		// If x is not seen so far or x is seen only once. then
		// repeated[x] is false
		final boolean[] repeated = new boolean[MAX_CHAR];

		// Let us consider following stream and see the process
		final String stream = "geeksforgeeksandgeeksquizfor";
